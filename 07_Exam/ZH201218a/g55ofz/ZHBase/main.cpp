...
void exitProgram()
{
	...
	std::cout << "Kilepeshez nyomj meg egy billetyut..." << std::endl;
	std::cin.get();
}
...
int main( int argc, char* args[] )
{
	...
	if (win == 0)
	{
		std::cout << "[Ablak letrehozasa] Hiba az SDL inicializalasa kozben: " << SDL_GetError() << std::endl;
		return 1;
	}

	//
	// 3. lépés: hozzuk létre az OpenGL context-et - ezen keresztül fogunk rajzolni
	//

	SDL_GLContext	context	= SDL_GL_CreateContext(win);
	if (context == 0)
	{
		std::cout << "[OGL context letrehozasa] Hiba az SDL inicializalasa kozben: " << SDL_GetError() << std::endl;
		return 1;
	}	
	...
	if ( error != GLEW_OK )
	{
		std::cout << "[GLEW] Hiba az inicializalas soran!" << std::endl;
		return 1;
	}
	...
	if ( glVersion[0] == -1 && glVersion[1] == -1 )
	{
		...
		std::cout << "[OGL context letrehozasa] Nem sikerult letrehozni az OpenGL context-et! Lehet, hogy az SDL_GL_SetAttribute(...) hivasoknal az egyik beallitas helytelen." << std::endl;
	...
	// 
	{
		...
		if (!app.Init())
		{
			...
			SDL_GL_DeleteContext(context);
			SDL_DestroyWindow(win);
			std::cout << "[app.Init] Az alkalmazas inicializalasa kozben hibatortent!" << std::endl;
			return 1;
		}
