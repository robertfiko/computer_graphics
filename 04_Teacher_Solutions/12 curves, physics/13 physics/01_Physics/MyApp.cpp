...
bool CMyApp::Init()
{
	...
	*/
	...
	m_lines.reserve(m_particleCount * 2);
	...
	for (int i = 0; i < m_particleCount; ++i)
	{
		...
		m_lines.push_back(m_particlePos[i]);
		m_lines.push_back(m_particlePos[i] + m_particleVel[i]);
	}
	...
	m_linesVBO.BufferData(m_lines);
	m_linesVAO.Init({
		{CreateAttribute<0, glm::vec3, 0, sizeof(glm::vec3)>, m_linesVBO}
	});
...
void CMyApp::Update()
{
	...
	for (int i = 0; i < m_particleCount; ++i)
	{
		m_particleVel[i] += gravity * delta_time;
		...
		{
			if (glm::length(m_particlePos[i] - m_particlePos[j]) < 0.05)
			{
				std::cout << "COLLISION " << i << " " << j << std::endl;
				glm::vec3 temp = m_particleVel[i];
				m_particleVel[i] = m_particleVel[j] * energyRemaining;
				m_particleVel[j] = temp * energyRemaining;
			}
		}


		m_lines[2 * i] = m_particlePos[i];
		m_lines[2 * i + 1] = m_particlePos[i] + m_particleVel[i];
	...
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * m_particlePos.size() * 2, &(m_lines[0][0]));
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void CMyApp::Render()
{
	...
	m_linesVAO.Bind();
	glDrawArrays(GL_LINES, 0, m_particleCount * 2);
	...
	{
		ImGui::InputFloat("energy remaining", &energyRemaining, 0.01f, 1.0f);
