...
void CMyApp::Render()
{
	...
	int interval = (int)m_currentParam;
	glm::vec3 dir;
	glm::vec2 angle;

	if (m_controlPoints.size() - m_currentParam - 1  > 0.01)
	{
		dir = m_controlPoints[interval + 1] - Eval(m_currentParam);
		angle = DirectionToAngle(dir);
	}
	else
	{
		dir = m_controlPoints[interval] - m_controlPoints[interval - 1];
		angle = DirectionToAngle(dir);
	}
...
glm::vec3 CMyApp::Eval(float t)
{
	...
	if(interval == 0 || interval == m_controlPoints.size() - 2)
		return (1 - localT) * m_controlPoints[interval] + localT * m_controlPoints[interval + 1];
	else
		return Catmull(localT, m_controlPoints[interval - 1],
			m_controlPoints[interval],
			m_controlPoints[interval + 1],
			m_controlPoints[interval + 2]);
}

glm::vec2 CMyApp::DirectionToAngle(glm::vec3 dir)
{
	dir = glm::normalize(dir);
	float a = atan2(dir.x, dir.z);
	float b = acos(dir.y) - M_PI / 2;
	return glm::vec2(a, b);
}
