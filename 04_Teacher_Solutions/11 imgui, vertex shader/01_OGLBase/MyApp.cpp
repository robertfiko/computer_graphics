...
void CMyApp::InitPlane()
{
	...
	m_PlaneVao.Init(
		{
			{ CreateAttribute<0, glm::vec2, 0, sizeof(glm::vec2)>, m_PlaneVbo }
		}, m_PlaneIndices
	);
}
...
void CMyApp::Update()
{
	...
	if(playing) time += delta_time * timeScale;
...
void CMyApp::Render()
{
	...
	glm::mat4 planeWorld = glm::scale(glm::vec3(10, 1, 10));

	m_programPlane.SetUniform("MVP", viewProj * planeWorld);
	m_programPlane.SetUniform("world", planeWorld);
	m_programPlane.SetUniform("worldIT", glm::inverse(glm::transpose(planeWorld)));
	m_programPlane.SetUniform("Ld", lightColor);
	m_programPlane.SetUniform("time", time);
	...
	if (ImGui::Begin("kezelofelulet"))
	{
		...
		ImGui::ColorEdit3("directional light color", (float*)&lightColor);
	}
	ImGui::End();
}
