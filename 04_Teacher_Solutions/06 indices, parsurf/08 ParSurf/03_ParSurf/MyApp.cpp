...
{
	return glm::vec3(u, sin(2 * v*2*M_PI), -v);
}

glm::vec3 CMyApp::GetPosTube(float u, float v)
{
	u *= 2 * M_PI;
	return glm::vec3(sin(u), v, cos(u));
}

glm::vec3 CMyApp::GetPosTorus(float u, float v)
{
	u *= 2 * M_PI;
	v *= 2 * M_PI;
	...
	glm::vec3 result;
	result.x = (R + r * cos(u)) * cos(v);
	result.z = (R + r * cos(u)) * sin(v);
	result.y = r * sin(u);
...
bool CMyApp::Init()
{
	...
	for (int i=0; i<=N; ++i)
		for (int j=0; j<=M; ++j)
		{
			...
			vert[i + j*(N+1)].p = GetPosTorus(u, v);
			// vert[i + j*(N+1)].c = glm::normalize( vert[i + j*(N+1)].p );
			vert[i + j * (N + 1)].c = glm::vec3(u,v,0);
		}
	...
	Vertex circle[N];
	circle[0] = { glm::vec3(0, 0, 0), glm::vec3(1, 0, 0) };
	for (int i = 0; i < N - 1; i++)
	{
		float fi = i / (float)(N - 2) * 2 * M_PI;
		circle[i + 1] = { glm::vec3(sin(fi), 0, cos(fi)), glm::vec3(0, 0, 1) };
	}
	...
	glBufferData(GL_ARRAY_BUFFER,	// az aktív VBO-ba töltsünk adatokat
		...
		circle,	// errõl a rendszermemóriabeli címrõl olvasva
		GL_STATIC_DRAW);	// úgy, hogy a VBO-nkba nem tervezünk ezután írni és minden kirajzoláskor felhasnzáljuk a benne lévõ adatokat
...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3(cos(t) * 10, 10, sin(t) * 10), // honnan nézzük a színteret
		glm::vec3(0, 0, 0), // a színtér melyik pontját nézzük
		glm::vec3(0, 1, 0)); // felfelé mutató irány a világban
}
...
void CMyApp::Render()
{
	...
	*/
	m_matWorld = /*glm::scale(glm::vec3(4, 0.3f, 5)) **/ glm::mat4(1);
	...
	/*
	m_matWorld = glm::translate(glm::vec3(0, 1, 0));
	...
	m_matWorld = glm::rotate(glm::radians(180.0f), glm::vec3(1, 0, 0));
	mvp = m_matProj * m_matView * m_matWorld;
	glUniformMatrix4fv(m_loc_mvp,// erre a helyre töltsünk át adatot
		1,			// egy darab mátrixot
		GL_FALSE,	// NEM transzponálva
