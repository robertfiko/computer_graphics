...
GLuint loadShader(GLenum _shaderType, const char* _fileName)
{
	...
	if (loadedShader == 0)
	{
		fprintf(stderr, "Hiba a %s shader inicializálásakor (glCreateShader)!\n", _fileName);
		return 0;
	}
	...
	{
		fprintf(stderr, "Link hiba\nA %s shader fajl betoltese nem sikerult\n", _fileName);
		return 0;
	}
	...
	if (GL_FALSE == result)
	{
		...
		std::cout << "Forditasi hiba\nA " << _fileName << " shader fajl forditasokor fellepo hibak:" << '\n';
